...
class InvestmentSignal(BaseModel):
    ...
    'Сектор чипов откатился -5% за день после отчёта поставщика, но
    ...
    Пример: 'AI-capex гиперскейлеров структурно расширяет спрос на годы вперёд;
    $13bn уже подписано. Просадка — точка входа.'"""

    critic_bear: str = ""
    """🐻 Мнение медвежьего критика — 1-2 предложения. Конкретный аргумент ПРОТИВ.
    Пример: 'Сектор в перегретой 6-летней четверти. Заминка в поставках крошит
    тренд на 30% за неделю, как в 2018.'"""

    prediction: str = ""
    """🔮 Краткосрочный прогноз — что ОЖИДАЕТСЯ в ближайшие 1-4 недели.
    Конкретно: ближайший катализатор, целевой уровень/диапазон, что
    переключит режим. Пример (SMH): 'Жду консолидацию $44-48 до отчёта
    поставщика; пробой $50 на объёме открывает путь к $58.'
    1-2 предложения. Это самое ближнее будущее (trade horizon)."""

    prediction_mid: str = ""
    """🗓️ Среднесрочный прогноз 1-3 месяца. Тренд сектора + ключевые
    катализаторы (CPI, FOMC, earnings, OPEC+). Пример (SMH):
    '1-3 месяца жду диапазон $48-60 при стабильных AI-capex новостях;
    провал ниже $45 на новостях о перебоях в поставках.'"""

    prediction_long: str = ""
    """📅 Долгосрочный прогноз 1-3 года. Структурный тренд / инвест-теза.
    Пример (SMH): 'На 1-3 года сектор структурно поддержан ростом
    AI-инфраструктуры; целевой диапазон $80-110 при подтверждении
    нескольких крупных заказов ежегодно.'"""
    ...
    3.57-3.60'. For stocks: 'IBKR market buy 10 SMH shares after open'.
